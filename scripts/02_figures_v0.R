...
canopy.hist <- ggplot(plant.data, aes(x = avg.leaf.area)) +
...
coef <- round(coef(area.cover.lm2), 2)
rsq <- round(summary(area.cover.lm2)$r.squared, 2)

# Create plot
scatterplot1 <- ggplot(plant.data, aes(x = avg.canopy.cover, y = avg.leaf.area)) +
  ...
  labs(title = "Scatterplot Comparing Leaf Area to Canopy Cover \nwith a Simple Linear Trendline",
       x = "Canopy Cover (%)",
       y = "Leaf Area (cm^2)") +
  annotate("text", x = 55, y = 30, label = paste("y =", coef[2], "x +", coef[1])) +
  annotate("text", x = 70, y = 30, label = paste("R² =", rsq)) +
  theme_minimal() +
  theme(plot.title = element_text(size = 20, face = "bold"),
        axis.title = element_text(size = 14),
        axis.text = element_text(size = 12),
